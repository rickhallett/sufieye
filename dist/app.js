...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let lastHoveredNode = null; // Track the last hovered node to prevent unnecessary updates
  ...
  function initializeGraph() {
    ...
    const options = {
      ...
      interaction: {
        ...
        tooltipDelay: isMobileDevice ? 1000 : 300, // Slightly longer delay to prevent jitter
      ...
      physics: {
        ...
        stabilization: {
          enabled: true,
          iterations: 800, // Increased for better initial stability
          updateInterval: 25,
          fit: true
        },
        forceAtlas2Based: {
          gravitationalConstant: -35,
          centralGravity: 0.025, // Slightly increased to keep nodes more centered
          springLength: isMobileDevice ? 120 : 150,
          springConstant: 0.08, // Increased for more stability
          damping: 0.85, // Increased to reduce oscillations
          avoidOverlap: 0.5
        },
        minVelocity: 0.75, // Higher minimum velocity to reduce small jitters
        maxVelocity: 50,   // Limit maximum velocity
        timestep: 0.5,     // Slightly reduced for smoother simulation
        adaptiveTimestep: true // Allow the simulation to adapt for better performance
      },
      configure: {
        enabled: false
      }
      ...
      setTimeout(() => {
        try {
          ...
          network.setOptions({
            physics: {
              enabled: false,
              // Keep a bit of physics for hover effects but make them very subtle
              stabilization: {
                enabled: false
              },
              minVelocity: 5,  // Higher min velocity means small movements are ignored
              maxVelocity: 20  // Lower max velocity limits how fast nodes can move
            }
          });
          isStabilized = true;
        } catch (e) {
  ...
  function setupBasicEventHandlers() {
    ...
    // Track user interaction to temporarily disable hover effects
    ...
    network.on("dragEnd", function () {
      ...
      }, 300); // Delay to prevent immediate hover effects after drag
    });
    ...
    // Throttled handler for hover node event
    const throttledHoverNode = throttle(function (params) {
      // Skip hover effects if user is actively interacting with the graph
      if (isUserInteracting) return;

      try {
        const nodeId = params.node;
        // Skip if it's the same node we just processed
        if (lastHoveredNode === nodeId) return;
    ...
    }, 80); // Throttle to one execution per 80ms

    // Throttled handler for blur node event
    const throttledBlurNode = throttle(function (params) {
      try {
        const nodeId = params.node;
        if (lastHoveredNode === nodeId) {
    ...
    // Apply throttled handlers
    network.on('hoverNode', throttledHoverNode);
    network.on('blurNode', throttledBlurNode);
